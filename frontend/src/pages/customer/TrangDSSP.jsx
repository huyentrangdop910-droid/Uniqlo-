// frontend/src/pages/customer/TrangDSSP.jsx

//import React from 'react';
/*import React, { useState, useEffect } from 'react'; // THÊM: useState, useEffect
//import { useParams, useNavigate, Link } from 'react-router-dom';
import { useParams, Link , useNavigate } from 'react-router-dom';
// Sửa: Import các icon cần thiết
import { Heart, ShoppingCart, ChevronDown, Star ,Home, Search, User} from 'lucide-react';
import Footer from '../../components/Footer.jsx'; // (Đảm bảo tên file Footer của bạn đúng)
import { MOCK_COLLECTIONS, MOCK_PRODUCTS } from '../../duLieu.js';
...
// ... (các import khác)
import ProductCard from '../../components/ProductCard'; // <-- THÊM DÒNG NÀY



// Component Thanh Filter
...
// ===== Component Trang Chính (THAY THẾ BẰNG CODE NÀY) =====
const ProductListPage = () => {
  // Lấy cả hai tham số có thể có từ URL
  ...
  // SỬA: Logic lấy Tiêu đề VÀ Ảnh Banner
  let title;
  let bannerImageUrl; // Biến mới để chứa URL ảnh banner

  if (collectionId) {
    // CÁCH 1: Nếu link từ SLIDE (dùng ID)
    const collection = MOCK_COLLECTIONS.find(c => c.id === parseInt(collectionId));
    if (collection) {
      title = collection.title;
      bannerImageUrl = collection.imageUrl; // LẤY URL ẢNH BANNER TỪ SLIDE
    } else {
      title = "Sản phẩm";
    }
  } else if (name) {
    // CÁCH 2: Nếu link từ MENU (dùng Tên)
    title = name;
    // bannerImageUrl sẽ là 'undefined', nên sẽ không hiển thị ảnh
  } else {
    title = "Sản phẩm"; // Tiêu đề mặc định nếu lỗi
  }

  // Luôn luôn chỉ hiển thị sản phẩm của ID 1
  const products = MOCK_PRODUCTS.filter(p => p.collectionId === 1); 

  // --- Các hàm xử lý (handle clicks) ---
