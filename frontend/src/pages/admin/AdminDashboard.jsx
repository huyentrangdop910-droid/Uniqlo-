...
import { 
  ...
  DollarSign, Briefcase, Package,
  Plus, Edit, Trash2, X, UploadCloud, Image as ImageIcon
} from 'lucide-react';
import { 
...
const salesData = [{ name: 'Jan', thisYear: 18000 }, { name: 'Feb', thisYear: 16000 }];
...
const AdminDashboard = () => {
  ...
  const [modalType, setModalType] = useState(''); // 'USER' hoặc 'PRODUCT'
  const [modalMode, setModalMode] = useState('create'); // 'create', 'edit', 'view'
  
  // --- FORM DATA ---
  // Form cho Nhân viên
  const [userForm, setUserForm] = useState({
      id: null, username: '', password: '', fullName: '', email: '', phone: '', address: ''
  });

  // Form cho Sản phẩm
  const [productForm, setProductForm] = useState({
      id: null, name: '', price: 0, importPrice: 0, 
      description: '', imageUrl: '', sizes: 'S,M,L'
  });
  ...
  const fetchStaffs = async () => {
      try {
        ...
        const res = await fetch('http://localhost:8080/api/v1/users/staffs', {
  ...
  // --- HANDLER: UPLOAD ẢNH (Cho sản phẩm) ---
  const handleImageUpload = (e) => {
      const file = e.target.files[0];
      if (file) {
          const reader = new FileReader();
          reader.onloadend = () => {
              // Lưu chuỗi Base64 vào form
  ...
  // 1. Mở Modal NHÂN VIÊN
  const openUserModal = (mode, staff = null) => {
      setModalType('USER');
      setModalMode(mode);
      if (mode === 'create') {
          setUserForm({ id: null, username: '', password: '', fullName: '', email: '', phone: '', address: '' });
      } else {
          setUserForm({
              id: staff.id,
  ...
  const openProductModal = (mode, product = null) => {
      ...
      setModalMode(mode);
      if (mode === 'create') {
          setProductForm({ id: null, name: '', price: 0, importPrice: 0, description: '', imageUrl: '', sizes: 'S,M,L' });
      } else {
          setProductForm({
              id: product.id,
              name: product.name,
  ...
  const handleSaveUser = async (e) => {
      ...
      let url = 'http://localhost:8080/api/v1/users/staffs';
      let method = 'POST';
      if (modalMode === 'edit') {
          url = `http://localhost:8080/api/v1/users/${userForm.id}`;
          method = 'PUT';
      }
      ...
      try {
          ...
          if (res.ok) {
              alert(modalMode === 'create' ? "Thêm nhân viên thành công!" : "Cập nhật thành công!");
              setShowModal(false);
              fetchStaffs();
  ...
  const handleSaveProduct = async (e) => {
      ...
      const payload = {
          ...
          importPrice: parseInt(productForm.importPrice),
      ...
      let method = 'POST';
      if (modalMode === 'edit') {
          url = `http://localhost:8080/api/v1/products/${productForm.id}`;
          method = 'PUT';
      }
      ...
      try {
          ...
          if (res.ok) {
              alert(modalMode === 'create' ? "Thêm sản phẩm thành công!" : "Cập nhật thành công!");
              setShowModal(false);
              fetchProducts();
          } else { alert("Lỗi: " + await res.text()); }
      } catch (e) { alert("Lỗi server"); }
    ...
    <div className="admin-layout">
      ...
      <aside className="admin-sidebar">
        <div className="sidebar-brand">
            ...
            <div className="brand-info"><span className="brand-name">Rentzy</span><span className="brand-role">Admin</span></div>
        </div>
        <div className="sidebar-menu">
          ...
          <div className={`menu-item ${activeTab==='customers'?'active':''}`} onClick={()=>setActiveTab('customers')}><Users size={20}/> Khách Hàng</div>
        ...
        <div className="sidebar-bottom">
            <div className="menu-item logout-item" onClick={handleLogout}><LogOut size={20}/> <span>Đăng xuất</span></div>
        </div>
      </aside>

      <main className="admin-main">
        <header className="admin-header">
            <div className="search-box"><Search size={20}/></div>
            <div className="header-actions"><Bell size={20}/><div className="admin-avatar"><img src="https://i.pravatar.cc/150?u=admin" alt="Admin" title={adminName}/></div></div>
        </header>
        ...
        <div className="dashboard-content">
          ...
          {activeTab === 'dashboard' && (
            <div className="stats-grid">
               <div className="stat-card"><div className="stat-top"><div><p className="stat-label">DOANH THU</p><h3>$24k</h3></div><div className="stat-icon red"><DollarSign size={24} color="white"/></div></div></div>
               <div className="stat-card"><div className="stat-top"><div><p className="stat-label">NHÂN SỰ</p><h3>{staffList.length}</h3></div><div className="stat-icon green"><Users size={24} color="white"/></div></div></div>
               <div className="stat-card"><div className="stat-top"><div><p className="stat-label">KHO HÀNG</p><h3>{productList.length}</h3></div><div className="stat-icon orange"><Package size={24} color="white"/></div></div></div>
            </div>
          )}
             ...
             <div className="staff-container">
                <div className="content-header">
                    ...
                    <button className="btn-primary" onClick={() => openUserModal('create')}>
                        <Plus size={18} style={{marginRight:'8px'}}/> Thêm Nhân Viên
                    </button>
                </div>
                <div className="table-wrapper">
                    <table className="admin-table">
                        <thead>
                            <tr>
                                <th>ID</th><th>Họ Tên</th><th>Tài khoản</th><th>Email</th><th>SĐT</th><th>Vai trò</th><th>Hành động</th>
                            </tr>
                        </thead>
                        <tbody>
                            {staffList.map((staff) => (
                                <tr key={staff.id} onClick={() => openUserModal('view', staff)} className="clickable-row">
                                    <td>#{staff.id}</td>
                                    <td><div className="user-cell"><div className="user-avatar-small">{staff.fullName ? staff.fullName.charAt(0) : 'U'}</div>{staff.fullName}</div></td>
                                    <td>{staff.username}</td><td>{staff.email}</td><td>{staff.phoneNumber || staff.phone}</td>
                                    <td><span className="badge badge-staff">{staff.role}</span></td>
                                    <td>
                                        <div className="action-buttons">
                                            <button className="btn-action edit" onClick={(e) => { e.stopPropagation(); openUserModal('edit', staff); }}><Edit size={16}/> Sửa</button>
                                            <button className="btn-action delete-text" onClick={(e) => handleDeleteUser(e, staff.id)}><Trash2 size={16}/> Xóa</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
             ...
             <div className="staff-container">
                <div className="content-header">
                    ...
                    <button className="btn-primary" onClick={() => openProductModal('create')}>
                        <Plus size={18} style={{marginRight:'8px'}}/> Nhập Hàng Mới
                    </button>
                </div>
                <div className="table-wrapper">
                    <table className="admin-table">
                        <thead>
                            <tr>
                                <th>ID</th><th>Ảnh</th><th>Tên SP</th><th>Giá Nhập</th><th>Giá Bán</th><th>Lãi</th><th>Hành động</th>
                            </tr>
                        </thead>
                        <tbody>
                            {productList.map((p) => (
                                <tr key={p.id}>
                                    <td>#{p.id}</td>
                                    <td>
                                        <img src={p.imageUrls?.[0] || 'https://placehold.co/40'} 
                                            alt="" style={{width:'50px', height:'50px', objectFit:'cover', borderRadius:'4px', border:'1px solid #eee'}}/>
                                    </td>
                                    <td style={{fontWeight:'500'}}>{p.name}</td>
                                    <td style={{color:'#6b7280'}}>{formatCurrency(p.importPrice || 0)}</td>
                                    <td style={{color:'#10B981', fontWeight:'bold'}}>{formatCurrency(p.price)}</td>
                                    <td style={{color:'#6366F1'}}>{formatCurrency(p.price - (p.importPrice || 0))}</td>
                                    <td>
                                        <div className="action-buttons">
                                            <button className="btn-action edit" onClick={() => openProductModal('edit', p)}><Edit size={16}/> Sửa</button>
                                            <button className="btn-action delete-text" onClick={() => handleDeleteProduct(p.id)}><Trash2 size={16}/> Xóa</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            <div className="modal-overlay">
                <div className="modal-content">
                    <div className="modal-header">
                        <h3>
                            {modalType === 'USER' ? (modalMode==='create'?"Thêm Nhân Viên":"Thông tin Nhân Viên") : (modalMode==='create'?"Nhập Hàng":"Sửa Sản Phẩm")}
                        </h3>
                    ...
                    {modalType === 'USER' && (
                        <form onSubmit={handleSaveUser}>
                            <div className="form-grid">
                                <div className="form-group"><label>Username</label><input type="text" name="username" value={userForm.username} onChange={handleUserChange} disabled={modalMode!=='create'} required/></div>
                                {modalMode !== 'view' && <div className="form-group"><label>Mật khẩu</label><input type="password" name="password" value={userForm.password} onChange={handleUserChange} placeholder={modalMode==='edit'?'(Để trống nếu không đổi)':''} required={modalMode==='create'}/></div>}
                                <div className="form-group"><label>Họ tên</label><input type="text" name="fullName" value={userForm.fullName} onChange={handleUserChange} disabled={modalMode==='view'} required/></div>
                                <div className="form-group"><label>Email</label><input type="email" name="email" value={userForm.email} onChange={handleUserChange} disabled={modalMode==='view'} required/></div>
                                <div className="form-group"><label>SĐT</label><input type="text" name="phone" value={userForm.phone} onChange={handleUserChange} disabled={modalMode==='view'} required/></div>
                                <div className="form-group"><label>Địa chỉ</label><input type="text" name="address" value={userForm.address} onChange={handleUserChange} disabled={modalMode==='view'}/></div>
                            </div>
                            <div className="modal-footer">
                                <button type="button" className="btn-secondary" onClick={() => setShowModal(false)}>{modalMode==='view'?'Đóng':'Hủy'}</button>
                                {modalMode !== 'view' && <button type="submit" className="btn-primary">{modalMode==='create'?'Tạo':'Lưu'}</button>}
                            </div>
                        </form>
                    )}
                                    ...
                                    <div className="image-upload-box">
                                        {productForm.imageUrl ? (
                                            <div className="image-preview">
                                                ...
                                                <button type="button" className="remove-img-btn" onClick={() => setProductForm({...productForm, imageUrl: ''})}>
                                                    <X size={14}/> Xóa ảnh
                                                </button>
                                            </div>
                                        ) : (
...
import { 
  LayoutDashboard, Users, LogOut, Bell, Search, 
  Briefcase, Package, UserCheck, // Import icon UserCheck cho tab Khách hàng
  Plus, Edit, Trash2, X, UploadCloud, Eye // Import Eye để xem chi tiết
} from 'lucide-react';
import { getUserProfile } from '../../services/authService';
import './AdminDashboard.css';

// --- HÀM HỖ TRỢ: Tự động đoán mã màu ---
...
const AdminDashboard = () => {
  ...
  const [staffList, setStaffList] = useState([]); 
  const [productList, setProductList] = useState([]); 
  const [customerList, setCustomerList] = useState([]); // State danh sách khách hàng
  
  // --- STATE MODAL ---
  const [showModal, setShowModal] = useState(false);
  const [modalType, setModalType] = useState(''); // 'USER', 'CUSTOMER', 'PRODUCT'
  const [modalMode, setModalMode] = useState('create'); // 'create', 'edit', 'view'
  ...
  const [productForm, setProductForm] = useState({
      ...
      description: '', imageUrl: '', sizes: 'S,M,L', colors: 'Trắng, Đen'
  });
  ...
  // 3. Lấy Khách hàng (MỚI)
  const fetchCustomers = async () => {
    try {
      const token = localStorage.getItem('userToken');
      // Gọi endpoint mới mà bạn vừa thêm ở Backend
    ...
    } catch (e) { console.error("Lỗi lấy khách hàng:", e); }
  };
    ...
    const checkAccess = async () => {
      ...
      try {
        ...
        if (profile.role !== 'ADMIN') { alert("Không có quyền!"); navigate('/'); } 
            ...
            fetchStaffs();    
            fetchProducts();
            fetchCustomers(); // Gọi hàm lấy khách hàng
        }
      } catch { localStorage.removeItem('userToken'); navigate('/admin'); }
    };
    checkAccess();
  ...
  const openUserModal = (mode, staff = null) => {
      ...
      if (mode === 'create') {
          ...
          setUserForm({
              ...
              fullName: staff.fullName,
              email: staff.email, 
              phone: staff.phoneNumber || staff.phone, // Kiểm tra cả phoneNumber (Backend trả về)
              address: staff.address || ''
          });
      }
      setShowModal(true);
  ...
  // 2. Modal Khách hàng (Xem chi tiết)
  const openCustomerModal = (customer) => {
    setModalType('CUSTOMER'); 
    setModalMode('view'); // Chế độ chỉ xem
    setUserForm({
        id: customer.id, 
        ...
        phone: customer.phoneNumber, // Map đúng trường phoneNumber từ UserProfileResponse
  ...
  const handleSaveUser = async (e) => {
      e.preventDefault();
      // Nếu là Form Khách hàng thì không cho lưu (vì chỉ xem)
      if (modalType === 'CUSTOMER') { setShowModal(false); return; }

      const token = localStorage.getItem('userToken');
      const payload = {
          username: userForm.username, password: userForm.password,
          fullName: userForm.fullName, email: userForm.email,
          phone: userForm.phone, address: userForm.address
      };
  ...
  const handleSaveProduct = async (e) => {
      ...
      const colorList = productForm.colors.split(',').map(c => {
          const colorName = c.trim();
          return { name: colorName, code: getColorHex(colorName) };
      }).filter(c => c.name !== '');
      ...
      try {
          ...
          if (res.ok) {
              alert("Lưu sản phẩm thành công!"); setShowModal(false); fetchProducts();
          } else { alert("Lỗi: " + await res.text()); }
      } catch (e) { alert("Lỗi server"); }
  };
  ...
  return (
    <div className="admin-layout">
      ...
      <aside className="admin-sidebar">
        <div className="sidebar-brand"><div className="brand-logo">R</div><div className="brand-info"><span className="brand-name">Rentzy</span><span className="brand-role">Admin</span></div></div>
        <div className="sidebar-menu">
          <div className={`menu-item ${activeTab==='dashboard'?'active':''}`} onClick={()=>setActiveTab('dashboard')}><LayoutDashboard size={20}/> Dashboard</div>
          <div className={`menu-item ${activeTab==='staff'?'active':''}`} onClick={()=>setActiveTab('staff')}><Briefcase size={20}/> Nhân Sự</div>
            ...
            <div className="stats-grid">
               ...
               <div className="stat-card">
                 <div className="stat-top">
                   ...
                   <div className="stat-icon orange"><Package size={24} color="white"/></div>
                 </div>
               </div>
               ...
               <div className="stat-card">
                 <div className="stat-top">
                   ...
                   <div className="stat-icon blue"><Users size={24} color="white"/></div>
                 </div>
               </div>
               ...
               <div className="stat-card">
                 <div className="stat-top">
                   <div>
                     <p className="stat-label">NHÂN SỰ</p>
                     <h3>{staffList.length}</h3>
                   </div>
                   <div className="stat-icon green"><Briefcase size={24} color="white"/></div>
                 </div>
               </div>
            </div>
          )}
                        ...
                        <tbody>
                            {staffList.map((staff) => (
                                <tr key={staff.id} onClick={() => openUserModal('view', staff)} className="clickable-row">
                                    <td>#{staff.id}</td>
                                    <td><div className="user-cell"><div className="user-avatar-small">{staff.fullName ? staff.fullName.charAt(0) : 'U'}</div>{staff.fullName}</div></td>
                                    <td>{staff.username}</td><td>{staff.email}</td>
             ...
             <div className="staff-container">
                ...
                <div className="table-wrapper">
                    <table className="admin-table">
                        <thead>
                            <tr>
                                <th>ID</th><th>Họ Tên</th><th>Email</th><th>Số điện thoại</th><th>Địa chỉ</th><th>Hành động</th>
                            </tr>
                        </thead>
                        <tbody>
                            {customerList.length > 0 ? customerList.map((cus) => (
                                <tr key={cus.id} onClick={() => openCustomerModal(cus)} className="clickable-row">
                                    <td>#{cus.id}</td>
                                   
                                    <td>
                                        <div className="user-cell">
                                            <div className="user-avatar-small" style={{backgroundColor: '#3B82F6'}}>
                                                {cus.fullName ? cus.fullName.charAt(0).toUpperCase() : 'C'}
                                            </div>
                                            {cus.fullName || cus.username}
                                        </div>
                                    </td>
                                    <td>{cus.email}</td>
                                    ...
                                    <td>
                                        <div className="action-buttons">
                                            <button className="btn-action view" onClick={(e) => { e.stopPropagation(); openCustomerModal(cus); }} title="Xem chi tiết">
                                                <Eye size={16} /> Xem
                                            </button>
                                        </div>
                                    </td>
                                ...
                                <tr><td colSpan="6" style={{textAlign: 'center', padding: '20px', color: '#888'}}>Chưa có khách hàng nào</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                        ...
                        <tbody>
                            {productList.map((p) => (
                                <tr key={p.id}>
                                    ...
                                    <td style={{fontWeight:'500'}}>{p.name}</td>
                                    <td style={{color: p.stockQuantity < 10 ? 'red' : 'inherit'}}>{p.stockQuantity}</td>
                                    <td>
                                        <div style={{display:'flex', gap:'4px'}}>
                                            {p.colors && p.colors.map(c => (
                                                <div key={c.code} title={c.name} style={{width:'15px', height:'15px', borderRadius:'50%', backgroundColor: c.code, border:'1px solid #ddd'}}></div>
                                            ))}
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div className="action-buttons">
                                            <button className="btn-action edit" onClick={() => openProductModal('edit', p)}><Edit size={16}/></button>
                                            <button className="btn-action delete-text" onClick={() => handleDeleteProduct(p.id)}><Trash2 size={16}/></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
          ...
          {showModal && (
            <div className="modal-overlay">
                <div className="modal-content">
                    <div className="modal-header">
                        <h3>
                            {modalType === 'USER' ? (modalMode==='create'?"Thêm Nhân Viên":"Thông tin Nhân Viên") : 
                             modalType === 'CUSTOMER' ? "Chi Tiết Khách Hàng" :
                             (modalMode==='create'?"Nhập Hàng":"Sửa Sản Phẩm")}
                        </h3>
                        <button className="close-btn" onClick={() => setShowModal(false)}><X size={20}/></button>
                    </div>
                    ...
                    {(modalType === 'USER' || modalType === 'CUSTOMER') ? (
                        <form onSubmit={handleSaveUser}>
                            <div className="form-grid">
                                <div className="form-group"><label>Username</label>
                                    ...
                                    <input type="text" name="username" value={userForm.username} onChange={handleUserChange} disabled={modalMode!=='create' && modalType!=='CUSTOMER'} required/>
                                </div>
                                
                               
                                {modalType === 'USER' && modalMode !== 'view' && (
                                     <div className="form-group"><label>Mật khẩu</label><input type="password" name="password" value={userForm.password} onChange={handleUserChange} placeholder={modalMode==='edit'?'(Để trống nếu không đổi)':''} required={modalMode==='create'}/></div>
                                )}

                                <div className="form-group"><label>Họ tên</label>
                                    <input type="text" name="fullName" value={userForm.fullName} onChange={handleUserChange} disabled={modalType === 'CUSTOMER' || modalMode==='view'} required/>
                                </div>
                                <div className="form-group"><label>Email</label>
                                    <input type="email" name="email" value={userForm.email} onChange={handleUserChange} disabled={modalType === 'CUSTOMER' || modalMode==='view'} required/>
                                </div>
                                <div className="form-group"><label>Số điện thoại</label>
                                    <input type="text" name="phone" value={userForm.phone} onChange={handleUserChange} disabled={modalType === 'CUSTOMER' || modalMode==='view'} />
                                </div>
                                <div className="form-group" style={{gridColumn: '1 / -1'}}><label>Địa chỉ</label>
                                    <input type="text" name="address" value={userForm.address} onChange={handleUserChange} disabled={modalType === 'CUSTOMER' || modalMode==='view'} />
                                </div>
                            </div>
                            ...
                            <div className="modal-footer">
                                {modalMode === 'view' ? (
                                    ...
                                    <>
                                        <button type="button" className="btn-secondary" onClick={() => setShowModal(false)}>Hủy</button>
                                        <button type="submit" className="btn-primary">{modalMode==='create'?'Tạo':'Lưu'}</button>
                                    </>
                                )}
                            </div>
                        </form>
                        ...
                        <form onSubmit={handleSaveProduct}>
                             <div className="form-grid">
                                <div className="form-group" style={{gridColumn: '1 / -1'}}><label>Tên SP</label><input type="text" name="name" value={productForm.name} onChange={handleProductChange} required /></div>
                                <div className="form-group" style={{gridColumn: '1 / -1'}}>
                                    <label>Hình Ảnh (Link hoặc Upload)</label>
                                    <div className="image-upload-box">
                                        {productForm.imageUrl ? (
                                            <div className="image-preview"><img src={productForm.imageUrl} alt="Preview" /><button type="button" className="remove-img-btn" onClick={() => setProductForm({...productForm, imageUrl: ''})}><X size={14}/></button></div>
                                        ) : (
                                            <label className="upload-btn"><UploadCloud size={24}/><span>Chọn ảnh</span><input type="file" accept="image/*" onChange={handleImageUpload} hidden /></label>
                                        )}
                                    </div>
                                </div>
                                <div className="form-group"><label>Giá Nhập</label><input type="number" name="importPrice" value={productForm.importPrice} onChange={handleProductChange} required /></div>
                                ...
                                <div className="form-group" style={{gridColumn: '1 / -1'}}>
                                    ...
                                    <input type="text" name="colors" value={productForm.colors} onChange={handleProductChange} placeholder="Ví dụ: Trắng, Đen"/>
                                </div>
...
import { 
  ...
  // Activity <-- Đã bỏ icon này vì không dùng nữa
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  // Dữ liệu giả lập cho biểu đồ truy cập (Vì chưa có tool đo lường thực tế)
  ...
  const calculateStats = () => {
      ...
      ordersList.forEach(order => {
          // 1. Tính biểu đồ đơn hàng theo tháng (Dựa vào ngày tạo đơn)
          if (order.createdAt) {
              const month = new Date(order.createdAt).getMonth();
              monthlyOrderCounts[month] += 1;
          }

          // 2. Chỉ tính doanh thu/lợi nhuận cho đơn ĐÃ GIAO
          if (order.status === 'Đã Giao Hàng') {
              revenue += order.totalAmount;

              // Tính giá vốn (để tính lợi nhuận)
              if (order.items) {
                  order.items.forEach(item => {
                      // Tìm sản phẩm gốc để lấy giá nhập (importPrice)
    ...
    //Slidebar
      ...
      <main className="admin-main">
        ...
        <div className="dashboard-content">
          ...
          {activeTab === 'inventory' && (
             <div className="staff-container">
                <div className="content-header"><h2>Quản Lý Kho Hàng</h2><button className="btn-primary" onClick={() => openProductModal('create')}><Plus size={18} style={{marginRight:'8px'}}/> Nhập Hàng Mới</button></div>
                <div className="table-wrapper">
                    <table className="admin-table">
                        <thead><tr><th>ID</th><th>Ảnh</th><th>Tên SP</th><th>Tồn</th><th>Màu Sắc</th><th>Giá Bán</th><th>Hành động</th></tr></thead>
                        <tbody>{productList.map((p) => (<tr key={p.id}><td>#{p.id}</td><td><img src={p.imageUrls?.[0] || 'https://placehold.co/40'} alt="" style={{width:'40px', height:'40px', objectFit:'cover', borderRadius:'4px'}}/></td><td style={{fontWeight:'500'}}>{p.name}</td><td style={{color: p.stockQuantity < 10 ? 'red' : 'inherit'}}>{p.stockQuantity}</td><td><div style={{display:'flex', gap:'4px'}}>{p.colors && p.colors.map(c => (<div key={c.code} title={c.name} style={{width:'15px', height:'15px', borderRadius:'50%', backgroundColor: c.code, border:'1px solid #ddd'}}></div>))}</div></td><td>{formatCurrency(p.price)}</td><td><div className="action-buttons"><button className="btn-action edit" onClick={() => openProductModal('edit', p)}><Edit size={16}/></button><button className="btn-action delete-text" onClick={() => handleDeleteProduct(p.id)}><Trash2 size={16}/></button></div></td></tr>))}</tbody>
                    </table>
                </div>
             </div>
          )}
...
import { 
  ...
  Briefcase, Package, UserCheck, 
  Plus, Edit, Trash2, X, UploadCloud, Eye, 
  DollarSign, TrendingUp, ShoppingBag, RotateCcw ,ChevronRight// <--- 1. Thêm RotateCcw
} from 'lucide-react';
import { 
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, 
  AreaChart, Area 
...
const [itemsPerPage] = useState(5); // Số lượng: 5 sản phẩm/trang (để bạn dễ test)

  // --- 2. STATE TÌM KIẾM KHO HÀNG (MỚI) ---
  ...
  // --- 3. HÀM TÌM KIẾM KHO HÀNG (MỚI) ---
  const handleSearchInventory = async () => {
    if (!inventorySearchTerm.trim()) {
        fetchProducts(); // Nếu ô tìm kiếm trống thì load lại tất cả
        return;
    }
    try {
        // Gọi API search: Gửi min=0 & max=2 tỷ để tránh lỗi tràn số ở Backend
...
<div className="search-bar-container" style={{
    ...
    <div className="search-input-wrapper" style={{position: 'relative', width: '350px'}}>
        ...
        <input 
            ...
            style={{
                width: '100%', 
                padding: '10px 10px 10px 40px', // Padding trái rộng để tránh icon
                borderRadius: '8px', 
    ...
    <button 
        ...
        style={{
            ...
            cursor: 'pointer',
            fontWeight: '500',
            flexShrink: 0 // Không cho nút bị co lại khi màn hình nhỏ
        }}
    >
        <Search size={16} />
        Tìm kiếm
    ...
    <button 
        ...
        onClick={() => { setInventorySearchTerm(''); fetchProducts(); }} 
        title="Làm mới danh sách"
        style={{
            height: '42px',
            width: '42px', // Nút vuông
            display: 'flex',
            alignItems: 'center',
